...
- Build/test automation gaps (root `ctest`, static analysis)
...
- P5 automation improvements (root `ctest`, `cppcheck`, sanitizer build option)
...
5. Build/test automation improvements
   - Added shared test labels and label filtering (`ctest -L shared`).
   - Added warnings-as-errors enforcement option (`ENABLE_WARNINGS_AS_ERRORS`).
   - Centralized cppcheck invocation via `tools/run_cppcheck.sh`.
   - Added strict cppcheck XML summary support (`tools/summarize_cppcheck_xml.py`).
...
- Continue periodic strict cppcheck runs and tighten gating policy as findings stabilize.
